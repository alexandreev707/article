...
class ChatController(
    ...
    fun sendMessage(@Valid @RequestBody dto: ChatMessageCreateDto): ResponseEntity<ChatMessageResponseDto> {
        val senderId = userService.getCurrentUserId() ?: throw IllegalStateException("User not authenticated")
    ...
    fun getConversation(
        ...
        val currentUserId = userService.getCurrentUserId() ?: throw IllegalStateException("User not authenticated")
        val messages = chatService.getConversation(
            currentUserId,
            UUID.fromString(userId),
            productId?.let { UUID.fromString(it) },
    ...
    fun markAsRead(@PathVariable messageId: String): ResponseEntity<Void> {
        val userId = userService.getCurrentUserId() ?: throw IllegalStateException("User not authenticated")
        chatService.markAsRead(UUID.fromString(messageId), userId)
